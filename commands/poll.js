...
exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  ...
  async function createPoll(message) {
    ...
    return [yesPollReactions,noPollReactions];
  }
  ...
  return new Promise((resolve) => {
    ...
    if (minutes < 1) {
      ...
      if (minutes < 1) {
        message.channel.send("Time must be at least 2 minutes.");
      }
    }
    ...
    var values = createPoll(message);

    var yesVotes = values[0];
    var noVotes = values[1];
        ...
        "fields": [
          {
            ...
            "value": (yesVotes.length - 2),
            "inline": true
          },
          {
            "name": "Total <:BotListNo:710291890465144872> votes:",
            "value": (noVotes.length - 2),
            "inline": true
          }
        ]
      };
