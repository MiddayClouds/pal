...
exports.run = async (client, message, [ action, key, ...value ],
  ...
  const taggedUser = message.guild.member(message.author)
  // if (taggedUser.hasPermission(['ADMINISTRATOR'])) {
  ...
  if (action === "edit") {
    ...
    const response = await client.awaitReply(
        message, `Are you sure you want to reset ${key} to the default value?`);
...
exports.help = {
  ...
  category : "System",
  description :
      "Allows users who have the right permissions to edit the bot's settings for the server. Settings include the prefix welcome message and much more. These must be first edited by the server owner. The `adminRole` and `modRole` settings should be the name of the role moderators and admins have respectively. The `modLogChannel` should be the name of the channel where logs should be sent if the `systemNotice` option is enabled. This will log when users try to use commands they should not be using.",
  usage :
      "To view the guild settings: `set` | To set a new option use: `set edit <option> [new setting]` | To reset an option use: `delete <option>`"
};
