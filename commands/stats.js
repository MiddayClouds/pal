...
exports.run = (client, message, args,
  ...
  // const duration = Timestamp.duration(client.uptime).format(" D [days], H
  // [hrs], m [mins], s [secs]");
  ...
  octokit.pulls
      ...
      .then((data) => {
        ...
        botInfo.setAuthor(client.user.username, client.user.displayAvatarURL(),
                          "https://github.com/MiddayClouds/pal");
        botInfo.setDescription(
            "*If you need help, use*  `" + message.settings.prefix +
            "help`\n[Github Repository](https://github.com/MiddayClouds/pal)\n[Command List](https://feen.us/9l5qhn)");
        botInfo.addField("\u200B", "\u200B", false);
        botInfo.addField("Guilds Serving:",
                         message.client.guilds.cache.size.toLocaleString(),
                         true);
        botInfo.addField("Users Serving:", client.getMembers(), true);
        botInfo.addField("\u200B", "\u200B", false);
        // This simply prints the latest github commit i shall add some more
        // like if it checks its open and stuff later..
        const latestSha = data.data[0].merge_commit_sha;
        botInfo.addField(
            "Commit Running:", "`" + latestSha.substring(0, 6) + "`", true);
